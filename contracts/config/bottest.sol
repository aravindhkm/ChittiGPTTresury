...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router,address _RWRD,address _WBNB) {
        RWRD = IBEP20(_RWRD);
        WBNB = _WBNB;
...
contract BotTest is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    constructor (address _router,address _reward,address _wbnb) Auth(msg.sender) {
        router = IDEXRouter(_router);
        WBNB = _wbnb;
        pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));
        _allowances[address(this)][address(router)] = uint256(-1);

        distributor = new DividendDistributor(address(router),_reward,_wbnb);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        console.log("token log --------->", amount);

        if (!authorizations[sender] && recipient != address(this)  && 
        recipient != address(DEAD) && recipient != pair && 
        recipient != marketingFeeReceiver && recipient != teamFeeReceiver 
         && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
        ...
        uint256 amountReceived = (!shouldTakeFee(sender) || !shouldTakeFee(recipient)) ?
         amount : takeFee(sender, amount,(recipient == pair));
        _balances[recipient] = _balances[recipient].add(amountReceived);

        console.log("wallet address", sender,recipient);
        console.log("after transfer from token ------>", amountReceived);
        // Dividend tracker
